/*
 * To run these tests on your project in VS, Go to File,
 * Create a new project, select C# and select UnitTest Project or TestProject if you are using VS2010.
 * (Make sure you have selected Add To Solution and not Create New Solution.)
 * After creation you should see the newly created project in the solution explorer.
 * Copy/Paste this file into the test project in the solution explorer
 * then right click on the References folder under the new Project and select Add Reference.
 * Go to either the Projects tab or the Solutions tab (Depending on which version of VS you are using)
 * and you should see your Set project, click on your Set project and add it.  You should see it appear
 * under the References folder in the solution explorer.  After that you'll need to add an include
 * for your Set's namespace above as specified.  Also if you're set is using generics then you will need
 * to modify this class appropriately.  After you have done this go to the Test tab, Run, Run all Tests.
 * This should run the tests, and then display the results on the left side of the IDE.  If you are
 * having problems with failing a test you can use stepping to walk through the test that you
 * are failing and see what's going wrong by placing a breakpoint, going to the Test tab, Debug,
 * Selected Test. (Make sure your cursor is in the test that you are debuging.  Alternatively you
 * can also click debug selected test from the test pane (May be easier to get to work properly).
 */

using System;
using Microsoft.VisualStudio.TestTools.UnitTesting;
using campbelljcscd371hw1;
